...
export const ProductionTable = (props: Props) => {
  ...
  const getRemainTodo = () => {
    const res =
      month.plan - month.workDays.reduce((acc, d) => acc + d.amount, 0);
    return res > 0 ? res : 0;
  };
  ...
  const getAvgWithPpr = () => {
    return Math.round(
      month.workDays.reduce((acc, d) => acc + d.amount, 0) /
        (month.workDays.length - month.ppr / 10 || 1)
    );
  };
  ...
  const getK = () => {
    const avg = Math.round(
      month.workDays.reduce((acc, d) => acc + d.amount, 0) ||
        0 / (month.workDays.length - month.ppr / 10 || 1)
    );
    if (month.product === Products.Napkins) return getKForNapkins(avg);

    if (month.product === Products.Toilets) return getKForToilets(avg);
  };
  ...
  return (
    <section className="production-table__table-container">
      <TableContainer
        component={Paper}
        className="production-table__main-table"
      >
        <Table stickyHeader>
          <TableHead>
            <TableRow>
            ...
            <TableRow>
              ...
              <TableCell className="production-table__cell" align="center">
                <span className="production-table__head-cell">
                  Среднее в смену
                </span>
              </TableCell>
            </TableRow>
