...
import { dataBase, mockMonthData } from "./repo";

export class DBService {
  private readonly url = dataBase.url;
  ...
  public async getMonth(p: Products): Promise<WorkMonth> {
    if (process.env.NODE_ENV !== "production") {
      return await JSON.parse(mockMonthData);
    }
  ...
  public async addWorkDay(product: Products, day: number, amount: number) {
    ...
    if (!res.ok) {
      throw new Error(
        `Could not update the work day` + `, received ${res.status}`
      );
    }
  }
  ...
  public async deleteWorkDay(product: Products, day: number) {
    ...
    if (!res.ok) {
      throw new Error(
        `Could not delete the work day` + `, received ${res.status}`
      );
    }
  }
}
